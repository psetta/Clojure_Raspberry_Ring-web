...
(def web_name "http://psetta-clojure.ga")
...
(defn generar_web [priv refresh titulo estilo contido] 
  (hiccup/html
   [:html {}
          ...
          [:body
           [:div {:id "frame"}
                 [:a {:href "/index"} [:div {:id "titulo"} "Psetta Maxima"]]
                 [:div {:id "indice"}
                       [:table
                        [:tr
                         (when priv
...
(defn handler [request]
  (def uri (request :uri))
  (if (float? (request :session))
      (def posibles ["/" "/sshlog" "/request" "/estilo.css" "/favicon.ico" "/index"])
      (def posibles ["/" "/estilo.css" "/favicon.ico" "/index"]))
